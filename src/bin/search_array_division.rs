...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    ...
    let k = unsafe { u32::to_posint(iter.next().unwrap_unchecked()) };
    let arr: Vec<_> = (0..n)
        .map(|_| unsafe { u64::to_posint(iter.next().unwrap_unchecked()) })
        .collect();

    let mut l = *arr.iter().max().unwrap();
    let mut r = arr.iter().sum::<u64>();

    while l < r {
        let mid = (l + r) >> 1;
        if sum_possible(&arr, k, mid) {
            r = mid;
        } else {
            l = mid + 1;
        }
