...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    ...
    let n = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) } + 1;
    let q = unsafe { u32::to_posint(iter.next().unwrap_unchecked()) };

    let mut forest: Vec<u32> = vec![0; MAX_FOREST_CAPACITY];
    for (row_idx, row) in (1..n).map(|idx| (idx, unsafe { iter.next().unwrap_unchecked() })) {
        for (cell, col_idx) in row.iter().zip(1..n) {
    ...
    for _ in 0..q {
        let y1 = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };
        let x1 = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };
        let y2 = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };
        let x2 = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };

        writer.maybe_flush(8);
        writer.add_int(unsafe {
