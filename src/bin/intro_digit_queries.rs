...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    ...
    let queries = unsafe { u16::to_posint(iter.next().unwrap_unchecked()) };
    for _ in 0..queries {
        let query_position = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };
        ...
        // not flushing here because total size of output is small.
        #[allow(clippy::cast_possible_truncation)]
        writer.add_int((target_number / (10_usize.pow(nth_digit as u32))) % 10);
        writer.add_byte(b'\n');
    }
}
