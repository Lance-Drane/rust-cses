...
/// A <i>border</i> of a string is a prefix that is also a suffix of the string but not the whole string. For example, the borders of <code>abcababcab</code> are <code>ab</code> and <code>abcab</code>.
///
...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    ...
    // z algorithm
    let (mut l, mut r) = (1, 1);
    let mut z_table = vec![n; n];
    for i in 1..n {
        let z_l = unsafe { *z_table.get_unchecked(i - l) };
        if r > i + z_l {
            unsafe {
                *z_table.get_unchecked_mut(i) = z_l;
            }
        } else {
            // update z box
            l = i;
            while r < i
                || (r < n && unsafe { *text.get_unchecked(r - i) == *text.get_unchecked(r) })
            {
                r += 1;
            }
            unsafe {
                *z_table.get_unchecked_mut(i) = r - i;
            }
        }
    }

    // output
    for (_, zlen) in z_table
        .iter()
        .enumerate()
        .rev()
        .take(n - 1)
        .filter(|(idx, len)| **len == n - *idx)
    {
