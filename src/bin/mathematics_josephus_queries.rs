...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    ...
    for _ in 0..q {
        let mut n = unsafe { u32::to_posint(iter.next().unwrap_unchecked()) };
        let mut k = unsafe { u32::to_posint(iter.next().unwrap_unchecked()) };

        let mut step = 2;
        let mut smallest = 1;

        writer.maybe_flush(11);
        writer.add_int(loop {
            let update_smallest = n & 1 == 1;
            n >>= 1;
            if n >= k {
                break smallest + (step >> 1) + step * (k - 1);
            }
            k -= n;
            if update_smallest {
                if k == 1 {
...
fn main() {
    let mut buf_str = Vec::with_capacity(440_007);
