...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    ...
    let values: Vec<_> = std::iter::once(0)
        .chain((0..n).map(|_| {
            sum ^= unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };
            sum
        }))
        .collect();
    ...
    for _ in 0..q {
        ...
        writer.add_int(unsafe {
            values.get_unchecked(usize::to_posint(iter.next().unwrap_unchecked()) - 1)
                ^ values.get_unchecked(usize::to_posint(iter.next().unwrap_unchecked()))
        });
        writer.add_byte(b'\n');
    }
}
