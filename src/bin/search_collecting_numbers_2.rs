...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    ...
    for (index, number) in (1..(n + 1)).map(|idx| {
        (idx, unsafe {
    ...
    for _ in 0..m {
        let l_idx = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };
        let r_idx = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };

        writer.maybe_flush(7);
        if l_idx == r_idx {
            writer.add_int(rounds);
            writer.add_byte(b'\n');
            continue;
        }
