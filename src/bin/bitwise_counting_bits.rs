...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    let mut num = unsafe { u64::to_posint(scan.get_unchecked(..scan.len() - 1)) };

    let mut ans = 0;

    while num != 0 {
        let bits = 63 - u64::from(num.leading_zeros());
        let shift = 1 << bits;
        num -= shift;
        // compute the answer for 2^<MOST_SIGNIFICANT_BIT>, but add "num" to correctly couple the computation with the "next" computation
        ans += num + bits * (shift >> 1) + 1;
    }
