...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    ...
    for _ in 0..t {
        let y = unsafe { u64::to_posint(iter.next().unwrap_unchecked()) };
        let x = unsafe { u64::to_posint(iter.next().unwrap_unchecked()) };
        let max = x.max(y);

        writer.maybe_flush(12);
        writer.add_int(if max & 1 == 0 {
