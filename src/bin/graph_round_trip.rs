...
fn solve<R: Read, W: Write>(mut reader: R, out: &mut W) {
    const MAX_INPUT_SIZE: usize = 2_800_014;
    let mut read_buf = vec![0_u8; MAX_INPUT_SIZE];
    let mut iter = {
        let read_size = reader.read(&mut read_buf).unwrap();
        let (scanned, _) = read_buf.split_at(read_size - 1);
        scanned.split(|&n| n <= b' ')
    };
    let mut writer = CustomBufWriter::new(out);

    let n_cities = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };
    let n_roads = unsafe { usize::to_posint(iter.next().unwrap_unchecked()) };
    let mut graph = Graph::new(n_cities + 1, n_roads << 1);
    ...
    let mut visited = vec![0_u64; (n_cities >> 6) + 1];
    let mut stack = Vec::with_capacity(n_cities);
    let mut iter_stack = Vec::with_capacity(n_cities); // keeps track of neighbors checked in a single call stack
    let mut first_seen; // only true when the stack was pushed to, false if backtracking or ignoring a neighbor

    for start_city in 1..(n_cities + 1) {
        let arr_idx = (start_city - 1) >> 6;
        let bit_idx = (start_city - 1) & 63;
        if (visited[arr_idx] >> bit_idx) & 1 == 0 {
            stack.push((start_city, 0));
            iter_stack.push(graph.adj_list(start_city));
            first_seen = true;
            while let Some((curr_node, prev_node)) = stack.last() {
                let neigh_iter = unsafe { iter_stack.last_mut().unwrap_unchecked() };
                if first_seen {
                    let visit = &mut visited[(*curr_node - 1) >> 6];
                    let visit_bit = (*curr_node - 1) & 63;
                    if (*visit >> visit_bit) & 1 == 1 {
                        // found solution
                        let count = stack.iter().rev().position(|x| x.1 == *curr_node).unwrap();
                        writer.add_int(count + 2);
                        writer.add_byte(b'\n');
                        writer.add_int(*curr_node);
                        writer.add_byte(b' ');
                        for (_, node) in stack.iter().rev().take(count + 1) {
                ...
                if let Some((_, next_node)) = neigh_iter.next() {
                    if next_node == *prev_node {
                        first_seen = false;
                    } else {
                        stack.push((next_node, *curr_node));
                        iter_stack.push(graph.adj_list(next_node));
                        first_seen = true;
                    }
                } else {
                    // backtrack
