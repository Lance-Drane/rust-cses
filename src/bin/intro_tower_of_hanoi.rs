...
fn solve<W: std::io::Write>(scan: &[u8], out: &mut W) {
    let discs = u8::to_posint(&scan[..scan.len() - 1]);
    let mut writer = CustomBufWriter::new(out);

    writer.add_int((1 << discs) - 1);
    writer.add_byte(b'\n');
    recurse(&mut writer, b'1', b'3', b'2', discs);
}

// supporting recursive function, just swap the positions around
fn recurse<W: std::io::Write>(
    writer: &mut CustomBufWriter<W>,
    from: u8,
    to: u8,
    swap: u8,
    disc: u8,
